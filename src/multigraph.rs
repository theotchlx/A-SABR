...
/// - `D`: A type implementing the `Distance<CM>` trait, which defines a method to measure distance
///   between nodes for routing purposes.
#[cfg_attr(feature = "debug", derive(Debug))]
pub struct Sender<NM: NodeManager, CM: ContactManager, D: Distance<CM>> {
    ...
    pub receivers: Vec<Receiver<NM, CM, D>>,
}
...
/// - `D`: A type implementing the `Distance<CM>` trait for measuring distance between nodes in routing contexts.

#[cfg_attr(feature = "debug", derive(Debug))]
pub struct Receiver<NM: NodeManager, CM: ContactManager, D: Distance<CM>> {
    /// The node represented by this receiver, wrapped in `Rc<RefCell<...>>`.
    pub node: Rc<RefCell<Node<NM>>>,
    /// A list of contacts providing paths to this receiver.
    pub contacts_to_receiver: Vec<Rc<RefCell<Contact<CM, D>>>>,
    /// The index of the next contact to be checked for relevance.
    pub next: usize,
}

impl<NM: NodeManager, CM: ContactManager, D: Distance<CM>> Receiver<NM, CM, D> {
...
#[cfg_attr(feature = "debug", derive(Debug))]
pub struct Multigraph<NM: NodeManager, CM: ContactManager, D: Distance<CM>> {
    /// * `senders` - The list of sender objects.
    pub senders: Vec<Sender<NM, CM, D>>,
...
impl<NM: NodeManager, CM: ContactManager, D: Distance<CM>> Multigraph<NM, CM, D> {
    ...
    pub fn new(mut nodes: Vec<Node<NM>>, mut contact_plan: Vec<Contact<CM, D>>) -> Self {
        // the contact plan might not be sorted
        // having a sorted list of contacts allow easy multigraph creation
        let mut data: Vec<Sender<NM, CM, D>> = Vec::new();
